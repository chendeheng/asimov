...
import (
	...
	"github.com/AsimovNetwork/asimov/crypto/bn256"
...
func RunPrecompiledContract(fvm *FVM, p PrecompiledContract, input []byte, contract *Contract) (ret []byte, err error) {
	...
	if contract.UseGas(gas) {
		return p.Run(fvm, input, contract)
	}
...
func (c *ecrecover) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
	...
	a, err := address.NewAddressPubKey(pubKey)
	a.SetFormat(address.PKFCompressed)
	
	return common.LeftPadBytes(a.AddressPubKeyHash().Bytes(), 32), nil
}
...
func (c *sha256hash) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
...
func (c *ripemd160hash) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
...
func (c *dataCopy) Run(fvm *FVM, in []byte, contract *Contract) ([]byte, error) {
	return in, nil
}
...
func (c *bigModExp) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
	var (
...
func (c *bn256Add) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
...
func (c *bn256ScalarMul) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
...
func (c *bn256Pairing) Run(fvm *FVM, input []byte, contract *Contract) ([]byte, error) {
...
func handleDelegateCall(systemContractAddr common.Address, fvm *FVM, input []byte, contract *Contract) (
	...
	instanceAddress := common.BytesToAddress(instanceAddr)
...
func IsSystemContract(address *big.Int) bool {
	if address.Cmp(MinSystemContractAddress) == -1 || address.Cmp(MaxSystemContractAddress) == 1 {
		return false
	}
	return true
}
